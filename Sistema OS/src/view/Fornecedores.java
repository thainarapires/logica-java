...
public class Fornecedores extends JDialog {
	...
	public Fornecedores() {
		...
		panel_1.setBounds(0, 405, 823, 38);
		...
		txtTel.setBounds(20, 269, 166, 29);
		...
		btnAdicionar.setBounds(389, 326, 68, 68);
		...
		btnApagar.setBounds(467, 326, 68, 68);
		...
		btnEditar.setBounds(310, 326, 68, 68);
		...
		btnNewButton.setBounds(753, 260, 60, 48);
	...
	private void adicionar() {
		...
		if (txtRazao.getText().isEmpty()) {
			...
			// lógica principal
			// CRUD Create
			String create = "insert into fornecedores (razaosocial,nomefantasia, cnpj, logradouro, numero, complemento, cep, bairro, referencia, cidade, uf,  telefone, celular , email) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
private void editar() {
		...
		if (txtRazao.getText().isEmpty()) {
			...
			// logica principal
			// CRUD - Update
			String update = "update fornecedores set razaosocial=?, nomefantasia=?, cnpj=?, logradouro=?, numero=?, complemento=?, cep=?, bairro=?, referencia=?, cidade=?, uf=?, telefone=?, celular=?, email=? where idfornecedor=?";
			// trat de exceção
			try {
				// abrir conexão
				con = dao.conectar();
